function Fig=PlotPerformance(X,Y,Fig)
%% Check Data
    I=isnan(X) | isnan(Y);
    X(I)=[];
    Y(I)=[];
...
if Fig.ParzenStimation
    ...
    if ~isfield(Fig,'ParzenValues')
        [Fig.ParzenValues.P,Fig.ParzenValues.PV]=ParzenWindow([Y,X],Fig.ParzenH);
    end
end
...
hold on
scatter3(X,Y,ones(size(Y)),Fig.ScatterSize,Fig.ParzenValues.PV,'s')
hold off
else
scatter(X,Y,'filled')
end
